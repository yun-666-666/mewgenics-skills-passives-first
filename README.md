...
- `SkillsPassivesFirst`（Mewjector DLL）改变 10 级及以上首次升级时的奖励候选，并在重掷时保留第三栏的新技能选项；
- `SkillsPassivesFirstData`（Mewtator 数据 MOD）为普通玩家猫提供 3 次升级重掷。

## 当前状态

此前版本的首次升级候选、三次重掷，以及基础职业、`Colorless` 和进阶职业覆盖已经完成玩家实际验收。本次新增的重掷保留第三栏技能行为仍需玩家游戏内验收。
...
“升级选项”使用游戏内置的主动技能/被动技能混合升级池，因此可能是两个主动、两个被动或各一个，取决于该猫仍可升级的内容。点击骰子重掷后，第三栏继续请求新主动技能，其余栏位默认使用原版奖励池。技能池耗尽时仍由游戏处理原生回退。
...
- `Enabled=1`：启用 DLL。
- `RerollKeepsPriority=0`：重掷时保留第三栏的新技能请求，其余栏位使用原版奖励池；设为 `1` 时全部栏位继续使用首次优先布局。
